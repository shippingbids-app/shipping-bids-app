    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          <h5 className="card-title">User name: <b>{user?.username}</b></h5>
          <h5 className="card-title">Email: <b>{user?.email}</b></h5>
          <h5 className="card-title">Phone number: <b>{user?.phoneNumber}</b></h5>
        </div>
        <ul className="list-group pt-3">
          <h3 className='ms-5 text-primary'>Offers: </h3>
          {user?.offers.map((offer) => (
            <li className="list-group-item" key={offer.id}>Offer: {offer.title}</li>
          ))}
        </ul>
        <ul className="list-group pt-3">
          <h3 className='ms-5 text-primary'>Services: </h3>
          {user?.services.map((service) => (
            <li className="list-group-item" key={service.id}>Service available on: <b>{service.address}</b></li>
          ))}
        </ul>
      </div>
    </div>
